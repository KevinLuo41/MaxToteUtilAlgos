...
###############################################################################
    ...
    depth  --> length     4
    height --> height     5
    TODO: refactor such that we dont assign vars like c1,c2,c3 etc... just have  return statements
          even tho bad practice to have multiple exit points, it will be faster
    '''
    def _canTakeProj_b(self, i, k, axis): # backwards
        ...
        res = False
        if (axis == Xy):
            c1 = (k.pos[y] + k.getL() <= i.pos[y])
            c2 = (i.pos[x]            <= k.pos[x] + k.getW())
            c3 = (k.pos[x] + k.getW() <  i.pos[x] + i.getW())
            c4 = (i.pos[z]            <= k.pos[z])
            c5 = (k.pos[z]            <  i.pos[z] + i.getH())
            if (c1 and c2 and c3 and c4 and c5): res = True
        elif (axis == Xz):
            c1 = (k.pos[z] + k.getH() <= i.pos[z])
            c2 = (i.pos[x]            <= k.pos[x] + k.getW())
            c3 = (k.pos[x] + k.getW() <  i.pos[x] + i.getW())
            c4 = (i.pos[y]            <= k.pos[y])
            c5 = (k.pos[y]            <  i.pos[y] + i.getL())
            if (c1 and c2 and c3 and c4 and c5): res = True

        elif (axis == Yz):
            c1 = (k.pos[z] + k.getH() <= i.pos[z])
            c2 = (i.pos[y]            <= k.pos[y] + k.getL())
            c3 = (k.pos[y] + k.getL() < i.pos[y] + i.getL())
            c4 = (i.pos[x]            <= k.pos[x])
            c5 = (k.pos[x]            <  i.pos[x] + i.getW())
            if (c1 and c2 and c3 and c4 and c5): res = True

        elif (axis == Yx):
            c1 = (k.pos[x] + k.getW() <= i.pos[x])
            c2 = (i.pos[y]            <= k.pos[y] + k.getL())
            c3 = (k.pos[y] + k.getL() < i.pos[y] + i.getL())
            c4 = (i.pos[z]            <= k.pos[z])
            c5 = (k.pos[z]            <  i.pos[z] + i.getH())
            ...
            c1 = (k.pos[x] + k.getW() <= i.pos[x])
            c2 = (i.pos[z]            <= k.pos[z] + k.getH())
            c3 = (k.pos[z] + k.getH() <  i.pos[z] + i.getH())
            c4 = (i.pos[y]            <= k.pos[y])
            c5 = (k.pos[y]            <  i.pos[y] + i.getL())
            if (c1 and c2 and c3 and c4 and c5): res = True
...
#########################_____STUFF FOR EXTREME POINTS____#####################
    ...
    def _canTakeProj_f(self, k, i, axis): # forward
        ...
        if (axis == Xy):
            ...
            c1 = (i.pos[z] + i.getH() <= k.pos[z])
            c2 = (i.pos[y]            <= k.pos[y] + k.getL())
            c3 = (k.pos[y] + k.getH() <  i.pos[y] + i.getL())
            c4 = (i.pos[x]            <= k.pos[x])
            c5 = (k.pos[x]            <  i.pos[x] + i.getW())
            if (c1 and c2 and c3 and c4 and c5): res = True
        elif (axis == Yx):
            c1 = (i.pos[x] + i.getW() <= k.pos[x])
            c2 = (i.pos[y]            <= k.pos[y] + k.getL())
            c3 = (k.pos[y] + k.getL() <  i.pos[y] + i.getL())
            c4 = (i.pos[z]            <= k.pos[z])
            c5 = (k.pos[z]            <  i.pos[z] + i.getH())
...
###############################################################################
    ...
    def _rotationChecks(self,sku,xp):
        ...
        all_dims = []
        ok_rots = []
        if isinstance(sku,Stack):
            FD = 2
        else:
            FD = 6
        for rot in range(FD): # remove rotations that are the same
            sku.modRot(rot)
            dims = sku.getDims() # when converting to java make sure to use the right method
            if (dims not in all_dims):
    ...
    def _tryAndScorePlacement(self, EPs, sku, b, sType):
        ...
        cur_fill = 0
        for xp in EPs:
            # print("Sku ",isinstance(sku, Stack))
            # print("stack ",not sku.getCanStackOnTop())
            # print("z", xp[Z_COORD] > 0)
            # print(xp)
            if isinstance(sku,Stack) and (not sku.getCanStackOnTop() and xp[Z_COORD] > 0):
                continue
            cur_xp = xp
    ...
    def _packToBox(self, b, skus,old_EPs, sType):
        ...
        if(len(old_EPs) == 0): # initialize the EPs
            ...
            # if isinstance(skus[0], Stack):
            #     for s in skus[0].getSkus():
            #         s.modPos(best_pos + s.getPos())
            #         s.modRot(best_rot)
            #         s.modDimsFromRot(best_rot)
